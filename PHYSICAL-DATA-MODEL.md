...
(System of Record)      (Telemetry)            (Audit & Events)

 asset.*               telemetry.*            application_logs
 configuration.*       Continuous Aggregates  TTL Indexes
 alert.*               Compression            JSON Documents
                        Retention Policies
```
---
...
| sensor_type | VARCHAR(30) | NOT NULL | Sensor classification |
| measured_value | DOUBLE PRECISION | NOT NULL | Measured sensor value |
| measurement_unit | VARCHAR(10) | NOT NULL | Unit of measurement |
| quality_status | VARCHAR(20) | NOT NULL | Reading quality indicator |
| ingestion_source | VARCHAR(20) | NOT NULL | Source of telemetry ingestion |
...
These indexes support the primary telemetry access patterns while minimizing write overhead.
---
...
To control long-term storage growth, telemetry data follows an automated retention policy.
...
This approach minimizes expensive aggregation queries on raw telemetry while improving dashboard and reporting performance.
---
...
|----------|----------|----------|
| event_identifier | String | Unique event identifier |
| correlation_identifier | String | End-to-end request correlation |
| device_identifier | String | Related device |
...
| exception | JSON Document | Exception details when applicable |
---
...
```json
{
  ...
  "event_timestamp": "2026-07-24T10:30:15Z",
...
| IX_logs_event_timestamp | Time-range filtering |
